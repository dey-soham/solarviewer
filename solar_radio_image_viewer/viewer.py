...
class SolarRadioImageTab(QWidget):
    ...
    def on_visualization_changed(self, colormap_name=None, dir_load=False):
        ...
        try:
            ...
            if dir_load:
                if "hmi" in os.path.basename(self.imagename).lower():
                    self.HMI_presets()
                elif "aia" in os.path.basename(self.imagename).lower():
                    if "94" in os.path.basename(self.imagename).lower():
                        self.aia_presets(wavelength=94)
                    elif "131" in os.path.basename(self.imagename).lower():
                        self.aia_presets(wavelength=131)
                    elif "171" in os.path.basename(self.imagename).lower():
                        self.aia_presets(wavelength=171)
                    elif "193" in os.path.basename(self.imagename).lower():
                        self.aia_presets(wavelength=193)
                    elif "211" in os.path.basename(self.imagename).lower():
                        self.aia_presets(wavelength=211)
                    elif "304" in os.path.basename(self.imagename).lower():
                        self.aia_presets(wavelength=304)
                    elif "335" in os.path.basename(self.imagename).lower():
                        self.aia_presets(wavelength=335)
                    elif "1600" in os.path.basename(self.imagename).lower():
                        self.aia_presets(wavelength=1600)
                    elif "1700" in os.path.basename(self.imagename).lower():
                        self.aia_presets(wavelength=1700)
                    elif "4500" in os.path.basename(self.imagename).lower():
                ...
                self.plot_image(
                    vmin_val, vmax_val, stretch, cmap, gamma, tight_layout=tight_layout
                )
    ...
    def aia_presets(self, wavelength=171):
        ...
        if wavelength == "4500":
            stretch = "linear"
        elif wavelength == "1700" or wavelength == "1600":
            stretch = "power"
            gamma = 0.7
        else:
            stretch = "arcsinh"
    ...
    def plot_image(
        ...
        # Apply tight_layout for better spacing and to avoid elements being cut off
        if tight_layout:
        ...
        <div class="container">
            ...
            <div class="section">
                ...
                <table>
                    ...
                    <tr><td><code>F9</code></td><td>HMI Presets</td></tr>
                    <tr><td><code>Ctrl+P</code></td><td>Phase Center Shift</td></tr>
