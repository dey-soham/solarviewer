...
def srs_to_pd(srs_file, pd_file, bkg_sub=False, do_flag=True, flag_cal_time=True):
    ...
    try:
        raw_output = subprocess.check_output(
            ["python", "srs_data.py", srs_file], universal_newlines=True
        )
...
def plot_learmonth_DS(pd_file, save_file="", start_time="", end_time=""):
    ...
    s = sns.heatmap(
        sel_data,
        robust=True,
        cbar_kws={"label": "Flux density (arbitrary unit)"},
        rasterized=True,
    )
    s.invert_yaxis()
...
def main():
    ...
    parser.add_option(
        "--flag", dest="flag", default=True, help="Perform flagging", metavar="Boolean"
    )
    parser.add_option(
        "--flag_caltime",
        dest="flag_caltime",
        default=True,
        help="Perform cal time flagging",
        metavar="Boolean",
    )
    parser.add_option(
        "--overwrite",
        dest="overwrite",
        default=False,
        help="Overwrite existing pd file",
        metavar="Boolean",
    )
    parser.add_option(
        "--plot_format",
