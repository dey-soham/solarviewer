...
"""def get_pixel_values_from_image(imagename, stokes, thres, rms_box=(0, 200, 0, 130)):
    stokes_map = {"I": 0, "Q": 1, "U": 2, "V": 3}

    # Demo mode if CASA is not available
    if not CASA_AVAILABLE:
        ...
        if stokes in ["I", "Q", "U", "V"]:
            idx = stokes_map.get(stokes, 0)
            pix = data[:, :, 0, 0] * (0.8 + 0.2 * idx)
            np.random.seed(idx)
            pix += np.random.normal(0, 0.05, pix.shape)
        elif stokes == "L":
            x = np.linspace(-5, 5, 512)
            y = np.linspace(-5, 5, 512)
            xx, yy = np.meshgrid(x, y)
            r = np.sqrt(xx**2 + yy**2)
            pix = np.exp(-((r - 2.5) ** 2) / 0.5)
        elif stokes in ["Lfrac", "Vfrac", "Q/I", "U/I", "U/V"]:
            x = np.linspace(-5, 5, 512)
            y = np.linspace(-5, 5, 512)
            xx, yy = np.meshgrid(x, y)
            pix = np.sin(xx) * np.cos(yy)
        else:
            pix = data[:, :, 0, 0]

        return pix, csys, psf

    # Normal mode (with CASA)
    ...
    ra_idx = np.where(dimension_names == "Right Ascension")[0][0]
    dec_idx = np.where(dimension_names == "Declination")[0][0]
    stokes_idx = np.where(dimension_names == "Stokes")[0][0]
    freq_idx = np.where(dimension_names == "Frequency")[0][0]
    if stokes_idx == 0:
    ...
    if stokes in ["I", "Q", "U", "V"]:
        ...
        slice_list_Q = [slice(None)] * len(data.shape)
        slice_list_U = [slice(None)] * len(data.shape)
        ...
        slice_list_Q = [slice(None)] * len(data.shape)
        slice_list_U = [slice(None)] * len(data.shape)
        slice_list_I = [slice(None)] * len(data.shape)
        slice_list_Q[stokes_idx] = 1
        slice_list_U[stokes_idx] = 2
        ...
        slice_list_V = [slice(None)] * len(data.shape)
        slice_list_I = [slice(None)] * len(data.shape)
