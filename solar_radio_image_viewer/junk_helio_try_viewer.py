...
try:
    # First check if sunpy is available
    import sunpy.map
...
class SolarRadioImageTab(QWidget):
    ...
    def setup_figure_toolbar(self, parent_layout):
        ...
        self.coord_system_action.triggered.connect(self.toggle_coordinate_system)
    ...
    def load_data(self, imagename, stokes, threshold):
        ...
        self.helioprojective_map = None  # Reset helioprojective map
        ...
        # Update the coordinate system toggle based on whether the image can be converted to helioprojective
        if hasattr(self, "coord_system_action"):
            if HELIOPROJECTIVE_AVAILABLE and self.can_convert_to_helioprojective():
                self.coord_system_action.setEnabled(True)
                self.coord_system_action.setToolTip(
                    "Toggle between RA/DEC and Helioprojective coordinates"
                )
            else:
                # If we're in helioprojective mode, switch back to RA/DEC
        ...
        if self.use_helioprojective and HELIOPROJECTIVE_AVAILABLE:
            try:
                ...
                if self.imagename:
                    ...
                    # Try to get the current Stokes parameter
                    stokes = "I"
                    if self.stokes_combo:
                        stokes = self.stokes_combo.currentText()

                    # Convert to helioprojective coordinates
                    self.helioprojective_map, cdelta_1, cdelta_2 = convert_to_hpc(
                        self.imagename,
                        Stokes=stokes,
                        lat=lat,
                        long=long,
