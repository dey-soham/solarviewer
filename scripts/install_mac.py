...
def install_mac_app():
    ...
    if not solarviewer_exec:
        print(
            "Error: Could not find 'solarviewer' executable. Please ensure it is installed (pip install .)."
        )
        return False
...
"""
    ...
    if icon_source.exists():
        ...
        try:
            ...
            print(
                "Warning: Could not convert icon using sips/iconutil. Copying PNG as fallback (might not work as expected)."
            )
